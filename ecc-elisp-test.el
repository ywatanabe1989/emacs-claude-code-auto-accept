...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-08 20:20:40>
;;; File: /home/ywatanabe/.dotfiles/.emacs.d/lisp/emacs-claude-code/ecc-elisp-test.el
...
(defun ecc-elisp-test-find-test-files (directory)
  ...
  (when (file-directory-p directory)
    (let ((files (directory-files-recursively directory "test-.*\\.el$" t)))
...
(defun ecc-elisp-test-find-tests-in-file (file)
  ...
  (with-temp-buffer
    ...
    (goto-char (point-min))
    (let (tests)
      (while (re-search-forward "^(ert-deftest\\s-+\\([^[:space:]\n]+\\)" nil t)
...
(defun ecc-elisp-test-run-single-test (file testname &optional timeout)
            ...
            (if (ert-test-boundp test-symbol)
                (with-timeout (timeout-secs
                               (list :name testname :passed nil 
                                     :message (format "TIMEOUT: Test exceeded time limit of %s" 
                                                     timeout-secs)))
                  (let* ((ert-debug-on-error nil) ; Prevent debugger from being triggered
                         (ert-result (ert-run-test test-symbol)))
                    (message "Test %s %s" 
                             testname 
                             (if (ert-test-passed-p ert-result) "PASSED" "FAILED"))
                    (list :name testname 
                          :passed (ert-test-passed-p ert-result)
                          :message (unless (ert-test-passed-p ert-result)
                                     (ert-test-result-with-explanation-message ert-result)))))
              (progn
                (message "Test %s NOT-FOUND" testname)
                (list :name testname :passed nil :message "NOT-FOUND: Test not found")))))
      (error
       (message "Test %s ERROR: %S" testname err)
       (list :name testname :passed nil :message (format "ERROR: %S" err))))))

(defun ecc-elisp-test-generate-report (test-results output-file &optional total-time-spent)
  ...
  (let* ((passed 0)
    ...
    (dolist (result test-results)
      (let ((test-name (plist-get result :name))
        ...
        (puthash test-name (1+ (gethash test-name test-names 0)) test-names)
        (cond
         (passed-p (cl-incf passed))
         ((and message (string-prefix-p "TIMEOUT:" message)) (cl-incf timeout))
         ((and message (string-prefix-p "NOT-FOUND:" message)) (cl-incf skipped))
         (t (cl-incf failed)))))
    
    ;; Count duplicates
    (maphash (lambda (k v) (when (> v 1) (cl-incf duplicates))) test-names)
    
    ;; Calculate success rate
    (let* ((total (+ passed failed skipped timeout))
           (success-rate (if (> total 0) (* 100.0 (/ (float passed) total)) 0.0))
      ...
      (with-temp-buffer
        ...
        (insert (format "- Timeout (= %d s): %d\n" ecc-elisp-test-timeout-sec timeout))
  ...
  (let* ((tests (ecc-elisp-test-find-tests-in-file file))
    ...
    (dolist (test tests)
      (let* ((file-path (car test))
             (test-name (cdr test))
             (result (ecc-elisp-test-run-single-test file-path test-name timeout-value)))
        (push result test-results)))

    ;; Calculate elapsed time
    (let ((total-time-spent (float-time (time-subtract (current-time) start-time))))
      ;; Generate the report
      (ecc-elisp-test-generate-report test-results output-file total-time-spent))
    
    (length tests)))

(defun ecc-elisp-test-run (directory &optional output-file timeout pattern)
  ...
  (let* ((test-files (ecc-elisp-test-find-test-files directory))
         ...
         (output-path (or output-file
                         (expand-file-name 
                          ...
                          (or ecc-elisp-test-default-output-dir directory))))
    ...
    (dolist (file filtered-files)
      (let ((tests (ecc-elisp-test-find-tests-in-file file)))
        (dolist (test tests)
          (let* ((file-path (car test))
                 (test-name (cdr test))
                 (result (ecc-elisp-test-run-single-test file-path test-name test-timeout)))
            (push result all-results)))))
    
    ;; Calculate elapsed time
    (let ((total-time-spent (float-time (time-subtract (current-time) start-time))))
      ;; Generate the report
      (ecc-elisp-test-generate-report all-results output-path total-time-spent)
