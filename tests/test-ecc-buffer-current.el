...
;;; Author: ywatanabe
;;; Timestamp: <2025-05-08 15:56:47>
;;; File: /home/ywatanabe/.dotfiles/.emacs.d/lisp/emacs-claude-code/tests/test-ecc-buffer-current.el
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'ecc-buffer-register-buffer) 
                     (lambda (buf) (add-to-list 'ecc-buffer-registered-buffers-alist (cons buf nil))))
                    ((symbol-function 'ecc-update-mode-line-all-buffers) 
                     #'ignore))
            
            ;; Set current buffer
            (should (eq test-buffer (ecc-buffer-set-current-buffer test-buffer)))
...
(ert-deftest test-ecc-buffer-current-buffer-state-functions ()
  ...
  (let ((original-current ecc-buffer-current-buffer)
        (original-registry ecc-buffer-registered-buffers-alist)
        (test-buffer (generate-new-buffer "*test-claude-current-state*")))
    (unwind-protect
        (progn
          ;; Setup clean test environment
          (setq ecc-buffer-registered-buffers-alist nil)
          ...
          (cl-letf (((symbol-function 'ecc-buffer-register-buffer) 
                    ...
                    ((symbol-function 'ecc-buffer-get-buffer-state) 
                     (lambda (buf) (cdr (assoc buf ecc-buffer-registered-buffers-alist))))
            ...
            (should (eq 'waiting (ecc-buffer-get-current-buffer-state)))
